...
config = {
    ...
    "batch_size": 10,
    "num_epochs": 5,
    "model_save_path": "noimage_caption_model.pth",
...
class RumorDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        encoded_text = self.tokenizer.encode_plus(
            text, add_special_tokens=True, return_tensors='pt', max_length=512, truncation=True, padding='max_length'
        )
...
import pandas as pd
def test_model(model, test_loader, experiment_name, fold_number, csv_path='noimage_caption_results.csv'):
    # 加载指定 fold 的最佳模型
    fold_model_save_path = config['model_save_path'].replace('.pth', f'_fold_{fold_number}.pth')
...
if __name__ == "__main__":
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset.samples)):
        ...
        # 测试模型
        fold_result = test_model(model, val_loader, experiment_name=f'fold_{fold+1}', fold_number=fold+1, csv_path='noiamge_caption_results.csv')
    ...
    avg_metrics_df.to_csv('avg_noimage_caption_results.csv', index=False)
