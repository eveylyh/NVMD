...
config = {
    ...
    "batch_size": 10,
    "num_epochs": 5,
    "model_save_path": "nofusion_model.pth",
...
class RumorDetectionModelWithoutAttention(nn.Module):
    def __init__(self):
        super(RumorDetectionModelWithoutAttention, self).__init__()
...
model = RumorDetectionModelWithoutAttention().to(device)
...
import pandas as pd
def test_model(model, test_loader, experiment_name, fold_number, csv_path='nofusion_results.csv'):
    # 加载指定 fold 的最佳模型
    fold_model_save_path = config['model_save_path'].replace('.pth', f'_fold_{fold_number}.pth')
...
if __name__ == "__main__":
    # 原始数据集
    dataset = RumorDetectionDataset(root_dir='E:/PycharmProjects/pmccRes/total_dataset', transform=transform)
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset.samples)):
        ...
        # 训练集和验证集的DataLoader
        train_dataset = RumorDetectionDataset(root_dir='E:/PycharmProjects/pmccRes/total_dataset', transform=transform)
        train_dataset.samples = train_samples
        train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, pin_memory=True)

        val_dataset = RumorDetectionDataset(root_dir='E:/PycharmProjects/pmccRes/total_dataset', transform=transform)
        ...
        # 测试模型
        fold_result = test_model(model, val_loader, experiment_name=f'fold_{fold+1}', fold_number=fold+1, csv_path='nofusion_results.csv')
    ...
    avg_metrics_df.to_csv('avg_nofusion_results.csv', index=False)
