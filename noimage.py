...
config = {
    ...
    "batch_size": 10,
    "num_epochs": 5,
    "model_save_path": "noimage_model.pth",
...
class RumorDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        text, picture_desc, _, label = self.samples[idx]
...
class RumorDetectionModel(nn.Module):
    def __init__(self):
        ...
        # 分类器
        self.classifier = nn.Linear(self.text_model.config.hidden_size, 2)  # 仅使用文本特征
...
import pandas as pd
def test_model(model, test_loader, experiment_name, fold_number, csv_path='noimage_results.csv'):
    # 加载指定 fold 的最佳模型
    fold_model_save_path = config['model_save_path'].replace('.pth', f'_fold_{fold_number}.pth')
...
if __name__ == "__main__":
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset.samples)):
        ...
        # 测试模型
        fold_result = test_model(model, val_loader, experiment_name=f'fold_{fold+1}', fold_number=fold+1, csv_path='noiamge_results.csv')
    ...
    avg_metrics_df.to_csv('avg_noimage_results.csv', index=False)
