...
class RumorDetectionDataset(Dataset):
    ...
    def _load_dataset(self):
        ...
        for label in ['fake', 'real']:
            label_dir = os.path.join(self.root_dir, 'twitter', label)
            for tweet_id in os.listdir(label_dir):
                tweet_path = os.path.join(label_dir, tweet_id)
                if os.path.isdir(tweet_path):
                    with open(os.path.join(tweet_path, 'content.json'), 'r') as file:
                        content = json.load(file)
                    text = content['text']
                    picture_desc = content['picture']

                    # 找到.jpg文件
                    img_files = [f for f in os.listdir(tweet_path) if f.endswith('.jpg')]
                    if img_files:
                        img_path = os.path.join(tweet_path, img_files[0])
                        samples.append((text, picture_desc, img_path, label))
        return samples
    ...
    def __getitem__(self, idx):
        text, picture_desc, img_path, label = self.samples[idx]

        # 直接拼接文本和图片描述
        combined_text = text + " " + picture_desc

        # 对拼接后的文本进行编码
        encoded_combined_text = self.tokenizer.encode_plus(
            combined_text, add_special_tokens=True, return_tensors='pt',
            max_length=512, truncation=True, padding='max_length'
        )
        input_ids = encoded_combined_text['input_ids'].squeeze(0)
        attention_mask = encoded_combined_text['attention_mask'].squeeze(0)
...
class RumorDetectionModel(nn.Module):
    def __init__(self):
        ...
        # 为了使图像特征的维度与文本特征相匹配
        self.image_feature_dim = self.text_model.config.hidden_size
        self.image_model.fc = nn.Linear(self.image_model.fc.in_features, self.image_feature_dim)

        # 协同注意力机制层
        self.text_attention = nn.Linear(self.image_feature_dim, 1)
        self.image_attention = nn.Linear(self.image_feature_dim, 1)

        # 分类器
        self.classifier = nn.Linear(self.image_feature_dim * 2, 2)  # 两倍特征维度，因为我们将会拼接它们
    ...
    def forward(self, input_ids, attention_mask, images):
        ...
        # 协同注意力机制
        text_attention_weights = torch.softmax(self.text_attention(image_features), dim=1)
        image_attention_weights = torch.softmax(self.image_attention(text_features), dim=1)

        attended_text_features = text_features * text_attention_weights
        attended_image_features = image_features * image_attention_weights
...
import pandas as pd
def test_model(model, test_loader, experiment_name, fold_number, csv_path='test_results.csv'):
    # 加载指定 fold 的最佳模型
    fold_model_save_path = config['model_save_path'].replace('.pth', f'_fold_{fold_number}.pth')
...
if __name__ == "__main__":
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(dataset.samples)):
        ...
        # 训练集和验证集的DataLoader
        train_dataset = RumorDetectionDataset(root_dir='total_dataset', transform=transform)
        train_dataset.samples = train_samples
        train_loader = DataLoader(train_dataset, batch_size=config['batch_size'], shuffle=True, pin_memory=True)

        val_dataset = RumorDetectionDataset(root_dir='total_dataset', transform=transform)
        ...
        # 测试模型
        fold_result = test_model(model, val_loader, experiment_name=f'fold_{fold+1}', fold_number=fold+1, csv_path='test_results.csv')
    ...
    avg_metrics_df.to_csv('average_results.csv', index=False)
